...
#   4. CLAUDE.md "Workflow routing"  -> corrected, openspec v1.5.0-aligned
...
#
# Idempotent: safe to re-run. Does NOT git commit (prints a suggested command at the end).
...
Usage: $0 [--locale zh-TW]
  --locale zh-TW   Use the Traditional Chinese CLAUDE.md fragment (default: en)
...
case "$LOCALE" in
  en)    FRAGMENT_NAME="CLAUDE.md.fragment.md" ;;
  zh-TW) FRAGMENT_NAME="CLAUDE.md.fragment.zh-TW.md" ;;
...
if [[ -n "$_src_dir" && -f "$_src_dir/templates/adopters/CLAUDE.md.fragment.md" ]]; then
...
echo "==> [5/6] Insert CLAUDE.md 'Workflow routing' section (locale=$LOCALE)"
...
# strip template meta (HTML comment lines) before inserting
fragment_body="${FRAGMENT_FILE}.body"
grep -v '^<!--' "$FRAGMENT_FILE" > "$fragment_body"

if [[ ! -f CLAUDE.md ]]; then
  cp "$fragment_body" CLAUDE.md
else
  # remove any existing "## Workflow routing ..." section (up to the next "## " heading)
  awk 'BEGIN{skip=0}
       /^## Workflow routing/{skip=1; next}
       skip==1 && /^## /{skip=0}
       skip==0{print}' CLAUDE.md > CLAUDE.md.tmp && mv CLAUDE.md.tmp CLAUDE.md
  # ensure a blank line separates, then append
  [[ -n "$(tail -c1 CLAUDE.md 2>/dev/null)" ]] && printf '\n' >> CLAUDE.md
  printf '\n' >> CLAUDE.md
  cat "$fragment_body" >> CLAUDE.md
fi
rm -f "$fragment_body"
