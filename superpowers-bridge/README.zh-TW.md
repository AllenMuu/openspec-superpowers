...
cd ~/your-project
bash /tmp/oss/superpowers-bridge/install.sh          # 完整安裝(冪等,不會 commit)
# 或逐步:
...
| 情境 | 是否要建 change | 怎麼做 |
|---|---|---|
| 新功能 / 新 capability | ✅ 要 | `/opsx:propose <name>`(預設 schema = superpowers-bridge) |
...
加上 `verify` artifact(apply step 3,透過 `openspec instructions verify` 產出 `verify.md`)。
...
### 快速流程(推薦)
```bash
/opsx:propose my-feature   # scaffold + 全部 planning artifacts(brainstorm/proposal/design/specs/tasks/plan)
/opsx:apply                # worktree + subagent-driven-development(TDD + code-review)+ verify.md
/opsx:archive              # 封存(retrospective 之後)
```

### 逐步流程
```bash
/opsx:propose my-feature   # 互動式:建立 change + 生成 planning artifacts 至 plan
/opsx:apply                # 實作 + worktree + subagent-driven-development + verify.md
# retrospective.md 在 verify 之後產出(evidence-first,§0 + 6 sections)
/opsx:archive
```

### 切回 spec-driven
```bash
# 單一 change 用不同 schema(CLI flag;/opsx:propose 用 repo 預設)
...
| 新 change | `/opsx:propose <name>` |
| 實作 change | `/opsx:apply <name>` |
| 歸檔 | `/opsx:archive <name>` |
| 檢視 change | `/opsx:explore <name>` |
| 把 specs 併回主線 | `/opsx:sync <name>` |
| 用 spec-driven(較輕量,跳過 brainstorm) | `openspec new change <name> --schema spec-driven` |
...
| Drift 通知 | [`version-check.yml`](../.github/workflows/version-check.yml) 每週,把基準 vs 最新 npm / GitHub release 字串比對 | Pinned ≠ latest upstream | 開 / 更新 [labelled drift issue](https://github.com/AllenMuu/openspec-superpowers/issues?q=is%3Aopen+label%3Aupstream-version-check),由人類檢核(workflow 維持綠 — drift 是正常狀態,不是錯誤) |
