...
加上一個 fallback:`superpowers:executing-plans`(apply step 2b)—— 只在無 subagent 支援的平台用;Claude Code 永遠走 2a。
...
skill 缺失 → STOP 並通知使用者,不靜默 fallback。使用者可選擇安裝 Superpowers,或明確 opt-in 走 instruction 末段的 manual fallback 路徑。
...
#### 2a. Executor — `superpowers:subagent-driven-development`(預設)
...
> **2b fallback**:只在無 subagent 支援的平台才用。Claude Code 有 subagent,所以 2a 永遠是對的。若被迫走 2b,TDD 與 code-review 不會傳遞觸發,要自己手動處理。
...
2b(`executing-plans`)存在,但被標為「無 subagent 支援平台」的 fallback,引用 Superpowers 自己的 SKILL.md L14。我們不發明「小 change 用 2b」這種自創規則。
...
若 Superpowers skill 不可用,每個相關 instruction 都包含明確的降級路徑:`brainstorm` → 手動;`plan` → 手動;apply → 標準 task-by-task。每個 instruction 開頭的 PRECHECK 確保缺失 skill 會清楚報錯,而非靜默漂移。
